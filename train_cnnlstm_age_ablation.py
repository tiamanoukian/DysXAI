...
# **Module docstring**
#
# CNN–LSTM training with Task 7 (hračkárstvo) only and A/B for the Age channel.
#
# Same protocol as ``train_cnn1d_age_ablation.py`` (K-fold, mean/std Val and Test AUC).
# Uses ``dysxai_tcn_cnnlstm_models.CNNLSTMModel`` (see ``03_model_cnnlstm.ipynb``).
#
# Run from project root:
#     python train_cnnlstm_age_ablation.py
#     python train_cnnlstm_age_ablation.py --k 5 --epochs 30 --no-comparison
...
from dysxai_tcn_cnnlstm_models import CNNLSTMModel
...
def main() -> None:
    p = argparse.ArgumentParser(description="CNN–LSTM K-fold (Task 7) with optional Age A/B comparison.")
    ...
    print("=" * 80)
    print("CNN–LSTM — Task 7 (hračkárstvo) only — Age channel A/B")
    print("=" * 80)
    ...
    def build_model(in_ch: int) -> CNNLSTMModel:
        return CNNLSTMModel(in_channels=in_ch)
